...
// - Parses AI Glossary.md into entries
...
async function init() {
  ...
  bindSearch();
  try {
    const mdText = await fetchMarkdown('AI Glossary.md');
    state.entries = parseGlossary(mdText);
    state.filtered = state.entries.slice();
    render();
  } catch (err) {
    showStatus(`Failed to load AI Glossary.md: ${err?.message || err}`);
  }
}
...
function stripMarkdownInline(str) {
  ...
  let s = str.replace(/\*\*(.*?)\*\*/g, '$1').replace(/\*(.*?)\*/g, '$1').replace(/`(.*?)`/g, '$1');
...
function applyFilters() {
  ...
  state.filtered = state.entries.filter(e => {
    const matchesLetter = byLetter === 'All' ? true : (e.term[0]?.toUpperCase() === byLetter);
...
function render() {
  ...
  // Empty state
  if (!shown) {
    showStatus('No entries match your filters. Try a different letter or search query.');
  } else {
    hideStatus();
  }
}
